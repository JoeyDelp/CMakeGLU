  ...
  for (unsigned i = 0; i < n; ++i) {
    ...
    for (unsigned j = 0; j < tmpcol2.size(); ++j) {
      ...
      if (nz_idx < i) {
        ...
        it = set_union(colhead, colend, tmpcol2.begin(), tmpcol2.end(),
                       tmpcol1.begin());
...
void Symbolic_Matrix ::csr() {
  ...
  for (unsigned i = 0; i < n; ++i) {
    for (unsigned j = 0; j < buffer[i].size(); ++j) {
      ...
      if (buffer[i][j] == i) csr_diag_ptr.push_back(csr_c_idx.size() - 1);
    }
...
void Symbolic_Matrix ::leveling() {
  ...
  for (unsigned i = 0; i < n; ++i) level_idx[tlen[inlevel[i]]++] = i;
...
void Symbolic_Matrix::ABFTCalculateCCA() {
  CCA.assign(n, 0.0);
  for (unsigned i = 0; i < n; ++i) {
    for (unsigned j = sym_c_ptr[i]; j < sym_c_ptr[i + 1]; ++j) CCA[i] += val[j];
  }
}
...
vector<REAL> Symbolic_Matrix::solve(SNicsLU *nicslu, const vector<REAL> &rhs) {
  ...
  if (mc64_scale)
    ...
    for (unsigned j = 0; j < n; ++j) b[j] = rhs[rp[piv[j]]];
  ...
  if (mc64_scale)
    for (unsigned j = 0; j < n; ++j) x[j] = b[cp[j]] * cols[j];
  else
    for (unsigned j = 0; j < n; ++j) x[j] = b[cp[j]];

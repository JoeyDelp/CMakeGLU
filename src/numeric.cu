...
__global__ void RL(const unsigned* __restrict__ sym_c_ptr_dev,
                   ...
                   REAL* __restrict__ tmpMem, const unsigned n,
                   const int levelHead, const int inLevPos) {
  const int tid = threadIdx.x;
  const int bid = blockIdx.x;
  const int wid = threadIdx.x / 32;

  const unsigned currentCol = level_idx_dev[levelHead + inLevPos + bid];
  const unsigned currentLColSize =
      sym_c_ptr_dev[currentCol + 1] - l_col_ptr_dev[currentCol] - 1;
  ...
  const unsigned subMatSize =
      csr_r_ptr_dev[currentCol + 1] - csr_diag_ptr_dev[currentCol] - 1;
  unsigned subCol;
  ...
  while (subMatSize > woffset) {
    if (wid + woffset < subMatSize) {
      ...
      while (offset < sym_c_ptr_dev[subCol + 1] - sym_c_ptr_dev[subCol]) {
        if (tidInWarp + offset <
            sym_c_ptr_dev[subCol + 1] - sym_c_ptr_dev[subCol]) {
...
__global__ void RL_perturb(const unsigned* __restrict__ sym_c_ptr_dev,
                           const unsigned* __restrict__ sym_r_idx_dev,
                           REAL* __restrict__ val_dev,
                           const unsigned* __restrict__ l_col_ptr_dev,
                           const unsigned* __restrict__ csr_r_ptr_dev,
...
__global__ void RL_onecol_factorizeCurrentCol(
    ...
    const unsigned* __restrict__ sym_r_idx_dev, REAL* __restrict__ val_dev,
    const unsigned* __restrict__ l_col_ptr_dev, const unsigned currentCol,
    REAL* __restrict__ tmpMem, const int stream, const unsigned n) {
  const int tid = threadIdx.x;
...
__global__ void RL_onecol_updateSubmat(
    ...
    const unsigned* __restrict__ sym_r_idx_dev, REAL* __restrict__ val_dev,
    const unsigned* __restrict__ csr_c_idx_dev,
    const unsigned* __restrict__ csr_diag_ptr_dev, const unsigned currentCol,
    REAL* __restrict__ tmpMem, const int stream, const unsigned n) {
  const int tid = threadIdx.x;
  const int bid = blockIdx.x;
  __shared__ REAL s;
...
void LUonDevice(Symbolic_Matrix& A_sym, ostream& out, ostream& err,
                bool PERTURB) {
  int deviceCount, dev;
  ...
  cudaSetDevice(dev);
  out << "Device " << dev << ": " << deviceProp.name << " has been selected."
      << endl;
  ...
  cudaMemcpy(sym_c_ptr_dev, &(A_sym.sym_c_ptr[0]), (n + 1) * sizeof(unsigned),
             cudaMemcpyHostToDevice);
  cudaMemcpy(sym_r_idx_dev, &(A_sym.sym_r_idx[0]), nnz * sizeof(unsigned),
             cudaMemcpyHostToDevice);
  cudaMemcpy(val_dev, &(A_sym.val[0]), nnz * sizeof(REAL),
             cudaMemcpyHostToDevice);
  cudaMemcpy(l_col_ptr_dev, &(A_sym.l_col_ptr[0]), n * sizeof(unsigned),
             cudaMemcpyHostToDevice);
  cudaMemcpy(csr_r_ptr_dev, &(A_sym.csr_r_ptr[0]), (n + 1) * sizeof(unsigned),
             cudaMemcpyHostToDevice);
  cudaMemcpy(csr_c_idx_dev, &(A_sym.csr_c_idx[0]), nnz * sizeof(unsigned),
             cudaMemcpyHostToDevice);
  cudaMemcpy(csr_diag_ptr_dev, &(A_sym.csr_diag_ptr[0]), n * sizeof(unsigned),
             cudaMemcpyHostToDevice);
  cudaMemcpy(level_idx_dev, &(A_sym.level_idx[0]), n * sizeof(int),
             cudaMemcpyHostToDevice);
  ...
  const size_t GoodtmpMemChoice =
      sizeof(REAL) * size_t(n) * size_t(A_sym.level_ptr[1]);
  ...
  for (int j = 0; j < Nstreams; ++j) cudaStreamCreate(&streams[j]);
  ...
  if (PERTURB) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      for (int j = A_sym.sym_c_ptr[i]; j < A_sym.sym_c_ptr[i + 1]; ++j)
        tmp += abs(A_sym.val[j]);
      if (norm_A < tmp) norm_A = tmp;
    }
    pert = 3.45e-4 * norm_A;
    out << "Gaussian elimination with static pivoting (GESP)..." << endl;
    out << "1-Norm of A matrix is " << norm_A << ", Perturbation value is "
        << pert << endl;
  }
  ...
  for (int i = 0; i < num_lev; ++i) {
    ...
    if (lev_size > 896) {  // 3584 / 4
      ...
      while (lev_size > 0) {
        ...
        if (!PERTURB)
          RL<<<dimGrid, dimBlock, MemSize>>>(
              sym_c_ptr_dev, sym_r_idx_dev, val_dev, l_col_ptr_dev,
              csr_r_ptr_dev, csr_c_idx_dev, csr_diag_ptr_dev, level_idx_dev,
              tmpMem, n, A_sym.level_ptr[i], j * TMPMEMNUM);
        else
          RL_perturb<<<dimGrid, dimBlock, MemSize>>>(
              sym_c_ptr_dev, sym_r_idx_dev, val_dev, l_col_ptr_dev,
              csr_r_ptr_dev, csr_c_idx_dev, csr_diag_ptr_dev, level_idx_dev,
              tmpMem, n, A_sym.level_ptr[i], j * TMPMEMNUM, pert);
        j++;
        lev_size -= TMPMEMNUM;
      }
        ...
        for (int j = 0; j < Nstreams; j++) {
          if (j + offset < lev_size) {
            const unsigned currentCol =
                A_sym.level_idx[A_sym.level_ptr[i] + j + offset];
            ...
            RL_onecol_cleartmpMem<<<1, 1024, 0, streams[j]>>>(
                sym_c_ptr_dev, sym_r_idx_dev, l_col_ptr_dev, currentCol, tmpMem,
                j, n);
          }
        }
      }
  ...
  cudaMemcpy(&(A_sym.val[0]), val_dev, nnz * sizeof(REAL),
